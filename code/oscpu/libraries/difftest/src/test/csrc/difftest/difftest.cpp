...
static const char *reg_name[DIFFTEST_NR_REG+1] = {
  "x00-$0",  "x01-ra",  "x02-sp",   "x03-gp",   "x04-tp",  "x05-t0",  "x06-t1",   "x07-t2",
  "x08-s0",  "x09-s1",  "x10-a0",   "x11-a1",   "x12-a2",  "x13-a3",  "x14-a4",   "x15-a5",
...
void Difftest::do_instr_commit(int i) {
  ...
  if (dut.commit[i].scFailed) {
    ...
    sync.lrscValid = 0;
    sync.lrscAddr = 0;
    proxy->uarchstatus_cpy((uint64_t*)&sync, DUT_TO_DIFFTEST); // sync lr/sc microarchitectural regs
  }
...
void Difftest::do_first_instr_commit() {
  // printf("has_commit:%d, cmt[0].valid:%d, cmt[0].pc:%lx\n", has_commit, dut.commit[0].valid, dut.commit[0].pc);
