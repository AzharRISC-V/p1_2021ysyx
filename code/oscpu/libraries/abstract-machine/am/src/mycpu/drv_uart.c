...
void virt_uart_init(void)
{
    ...
    uart_write_reg(IER, 0x00);                  // W 1, 0.  Disable interrupt

    uint8_t lcr = uart_read_reg(LCR);           // R 3, 3
    uart_write_reg(LCR, lcr | (1 << 7));        // W 3, 83. The divisor latches can be accessed.
    uart_write_reg(DLL, 0x01);                  // W 0, 1.  LSB of Divisor Latch
    uart_write_reg(DLM, 0x00);                  // W 1, 0.  MSB of Divisor Latch

    lcr = 0;
    uart_write_reg(LCR, lcr | (3 << 0));        // W 3, 3.  The divisor latches can not be accessed.
