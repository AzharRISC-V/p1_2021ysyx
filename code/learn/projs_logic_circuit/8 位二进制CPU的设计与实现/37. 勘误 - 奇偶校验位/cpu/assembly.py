...
INSTRUCTIONS = {
    ...
    1 : {
        ...
        CALL: {
            ...
            (pin.AM_INS) : [
                ...
                pin.SP_OUT | pin.MAR_IN,                # MAR = SP
                pin.SS_OUT | pin.MSR_IN,                # MSR = 栈段寄存器的值，确定了栈段
                pin.PC_OUT | pin.RAM_IN,                # PC读取，并写入内存
                # JMP xx
                pin.DST_OUT | pin.PC_IN,
                pin.CS_OUT | pin.MSR_IN,                # 段寄存器还原为代码段
            ],
            # 1. CALL A。相当于：PUSH PC, JMP A
            (pin.AM_REG) : [
                # PUSH PC
                pin.SP_OUT | pin.A_IN,                  # A = SP
                pin.OP_DEC | pin.SP_IN | pin.ALU_OUT,   # SP = SP - 1
        ...
        # 类似于 CALL。唯一的区别：在执行的最后，需要关中断，防止再次发生中断。
        INT: {
            # 1. INT xx。相当于：PUSH PC, JMP xx
            (pin.AM_INS) : [
                # PUSH PC
                pin.SP_OUT | pin.A_IN,                  # A = SP
                pin.OP_DEC | pin.SP_IN | pin.ALU_OUT,   # SP = SP - 1
                ...
                # JMP xx
                pin.DST_OUT | pin.PC_IN,
                pin.CS_OUT | pin.MSR_IN | pin.ALU_PSW | pin.ALU_CLI,     # 段寄存器还原为代码段，还要关中断
            ],
            # 1. CALL A。相当于：PUSH PC, JMP A
            (pin.AM_REG) : [
                # PUSH PC
    ...
    0 : {
        ...
        RET : [
            pin.SP_OUT | pin.MAR_IN,                # MAR = SP
            pin.SS_OUT | pin.MSR_IN,                # MSR = 栈段寄存器的值，确定了栈段
            pin.PC_IN | pin.RAM_OUT,                # 读取内存，写入目的寄存器读取。
            pin.SP_OUT | pin.A_IN,                  # A = SP
            pin.OP_INC | pin.SP_IN | pin.ALU_OUT,   # SP = SP + 1
            pin.CS_OUT | pin.MSR_IN,                # 段寄存器还原为代码段
        ],
        # IRET, 相当于 RET。区别是，执行到最后需要：开中断
        IRET : [
            pin.SP_OUT | pin.MAR_IN,                # MAR = SP
            pin.SS_OUT | pin.MSR_IN,                # MSR = 栈段寄存器的值，确定了栈段
            pin.PC_IN | pin.RAM_OUT,                # 读取内存，写入目的寄存器读取。
            pin.SP_OUT | pin.A_IN,                  # A = SP
            pin.OP_INC | pin.SP_IN | pin.ALU_OUT,   # SP = SP + 1
            pin.CS_OUT | pin.MSR_IN | pin.ALU_PSW | pin.ALU_STI,    # 段寄存器还原为代码段，开中断
        ],
        STI : [
